...
import "../encadrant/Sujet.css";
import axios from "axios";

function DashboardStage() {
  const [sujetList, setsujetList] = useState([]);
  useEffect(() => {
    axios.get("/api/allSujets").then((res) => {
      if (res.status === 200) {
        setsujetList(res.data.sujet);
      }
    });
  }, []);

  var viewAllSujets = "";
  viewAllSujets = sujetList.map((item, pos) => {
    return (
      <div class="col-lg-12 col-md-12 col-sm-12" key={pos}>
          ...
          <div class="solution_cards_box">
            <div class="solution_card">
              ...
              <div class="solu_description">
                ...
                <Link
                  to={`/stagiaire/editSujet?id=${item._id}`}
                  className="btn read_more_btn"
                >
                  Read more...
                </Link>
              </div>
  ...
  return (
    <StagiaireLayout>
      ...
      <div className="container">
        <br></br>
        <div class="row">{viewAllSujets}</div>
      </div>
    </StagiaireLayout>
  );
}
